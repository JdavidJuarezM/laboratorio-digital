...
const Medidor = memo(({nivel, tipo, respuestasCorrectas, totalPreguntas = 3}) => { // Default 3
    ...
    return (
        <div className="w-full mb-3">
            ...
            <div
                className="relative h-6 bg-gray-900/50 rounded-full border-2 border-white/30 backdrop-blur-sm overflow-hidden shadow-lg">
            ...
            {respuestasCorrectas !== undefined && (
                <div
                    className="text-center text-xs font-bold text-white mt-1 bg-black/30 rounded-full py-0.5 inline-block px-3 mx-auto w-full backdrop-blur-md border border-white/10">
                    {/* CAMBIO: Usamos totalPreguntas dinámico */}
                    🧠 Aciertos para crecer: <span
                    className="text-yellow-300">{respuestasCorrectas} / {totalPreguntas}</span>
                </div>
            )}
        </div>
    );
...
Medidor.propTypes = {
    ...
    totalPreguntas: PropTypes.number,
};
