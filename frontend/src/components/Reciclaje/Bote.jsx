...
const Bote = ({ id, label, icon, color }) => {
  ...
  return (
    <div ref={setNodeRef} id={id} className={containerClasses}>
      ...
      {/* Tapa trasera */}
      <div className={`absolute top-2 w-[90%] h-4 md:h-8 rounded-t-lg md:rounded-t-2xl brightness-75 ${color} z-0 translate-y-1 md:translate-y-2 transition-colors`}></div>

      {/* Cuerpo Principal */}
      <div className={bodyClasses}>

        {/* Apertura Oscura */}
        <div className="w-[80%] h-6 md:h-12 bg-slate-900/40 rounded-full shadow-inner mb-0.5 md:mb-2 relative overflow-hidden group">
        ...
        <div className="flex flex-col items-center justify-center gap-0 w-full px-1 flex-1">
            ...
            <div className={`text-3xl md:text-6xl filter drop-shadow-md transition-transform duration-300 ${isOver ? 'animate-bounce-custom' : ''}`}>
                {icon}
            </div>

            {/* Etiqueta: Grande en PC, Diminuta en móvil */}
            <div className="bg-black/20 px-1.5 py-0.5 md:px-3 md:py-1 rounded-md md:rounded-lg backdrop-blur-sm border border-white/10 w-full text-center mt-1">
                <span className="text-white font-black text-[8px] md:text-sm uppercase tracking-wider block leading-tight text-shadow-sm truncate">
                    {label}
                </span>
            </div>
        </div>
        ...
        {/* Brillo lateral */}
        <div className="absolute left-1 md:left-2 top-2 md:top-4 w-1 md:w-2 h-[80%] bg-gradient-to-b from-white/40 to-transparent rounded-full blur-[0.5px]"></div>
      </div>
    </div>
  );
};
