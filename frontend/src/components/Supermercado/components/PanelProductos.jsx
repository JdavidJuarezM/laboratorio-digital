    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-4 backdrop-blur-sm">
      <div className="bg-white w-full max-w-6xl h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-4 flex justify-between items-center text-white shadow-md z-10">
          ...
          <button onClick={onClose} className="text-white/80 hover:text-white text-2xl font-bold transition">&times;</button>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
               ...
               {isAddingCat ? (
                 <div className="animate-fadeIn">
                   ...
                   <div className="flex gap-2">
                     <button onClick={handleSaveCategory} className="flex-1 bg-green-500 text-white text-xs py-1 rounded">Guardar</button>
                     <button onClick={() => setIsAddingCat(false)} className="flex-1 bg-gray-200 text-xs py-1 rounded">Cancelar</button>
                   </div>
                 </div>
               ) : (
                 <button
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-slate-50">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 mb-8">
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                <div className="md:col-span-3">
                  ...
                  <input name="nombre" value={nuevoProducto.nombre} onChange={handleInputChange} className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-100 outline-none" placeholder="Ej: Manzana" required />
                </div>
                <div className="md:col-span-2">
                  <label className="text-xs font-bold text-gray-500 block mb-1">EMOJI</label>
                  <input name="emoji" value={nuevoProducto.emoji} onChange={handleInputChange} className="w-full p-2 border rounded-lg text-center text-xl focus:ring-2 focus:ring-blue-100 outline-none" placeholder="🍎" />
                </div>
                <div className="md:col-span-2">
                  <label className="text-xs font-bold text-gray-500 block mb-1">PRECIO</label>
                  <input name="precio" type="number" step="0.5" value={nuevoProducto.precio} onChange={handleInputChange} className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-100 outline-none" placeholder="0.00" required />
                </div>
                <div className="md:col-span-3">
                  <label className="text-xs font-bold text-gray-500 block mb-1">CATEGORÍA</label>
                  <select name="categoria" value={nuevoProducto.categoria} onChange={handleInputChange} className="w-full p-2 border rounded-lg bg-white focus:ring-2 focus:ring-blue-100 outline-none">
                    {listaCategorias.map(cat => <option key={cat} value={cat}>{cat}</option>)}
                  </select>
                </div>
                <div className="md:col-span-2">
                  <button type="submit" className="w-full bg-blue-600 text-white font-bold py-2 rounded-lg hover:bg-blue-700 shadow-md transition transform active:scale-95">
                    Guardar
                  </button>
                </div>
              </form>
            ...
            <div className="flex justify-between items-center mb-4">
               ...
               {productos.length < 5 && (
                 <button onClick={cargarDatosIniciales} className="text-xs text-blue-500 underline hover:text-blue-700">
            ...
            {loading ? <p className="text-center text-gray-400">Cargando...</p> : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {productosFiltrados.map((prod) => (
                  <div key={prod.id} className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center group hover:shadow-md transition">
                    <div className="flex items-center gap-3 overflow-hidden">
                      <span className="text-3xl bg-gray-50 p-2 rounded-lg">{prod.emoji || "📦"}</span>
                      <div className="min-w-0">
                        <p className="font-bold text-gray-800 truncate text-sm">{prod.nombre}</p>
                        <div className="flex items-center gap-2 text-xs">
                           <span className="bg-green-100 text-green-700 px-1.5 py-0.5 rounded font-bold">${prod.precio}</span>
                           <span className="text-gray-400 truncate max-w-[80px]">{prod.categoria}</span>
                        </div>
                      </div>
                    </div>
                    <button onClick={() => handleEliminar(prod.id)} className="text-gray-300 hover:text-red-500 hover:bg-red-50 p-2 rounded-lg transition">
                      🗑️
                    </button>
                  </div>
                ))}
