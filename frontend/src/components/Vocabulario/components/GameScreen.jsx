...
const TimerBar = ({ timer, timerDuration }) => {
  ...
  return (
    <div className="w-full h-3 md:h-4 lg:h-5 bg-gray-200 rounded-full my-1 overflow-hidden shadow-inner border border-gray-300">
      <div
...
const UserInputDisplay = ({ word, userInput }) => (
  <div className="flex justify-center gap-1 sm:gap-2 md:gap-3 flex-wrap items-center w-full min-h-[60px] md:min-h-[80px]">
    {word &&
      [...word].map((_, i) => (
        <div
          key={i}
...
const LetterTray = ({ letters, userInput, onLetterClick }) => (
  <div className="bg-purple-50 rounded-2xl p-2 sm:p-4 lg:p-6 flex justify-center flex-wrap gap-2 sm:gap-3 lg:gap-4 my-2 border-2 border-purple-100 w-full shadow-inner overflow-y-auto max-h-[30vh]">
...
const GameScreen = ({
  ...
  return (
    ...
    <div className="w-full h-full flex items-center justify-center p-2 sm:p-4">
      ...
      {/* Tarjeta principal */}
      <div className="card w-full max-w-screen-2xl h-full bg-white rounded-[1.5rem] sm:rounded-[2.5rem] shadow-2xl flex flex-col relative overflow-hidden border-4 border-white">

        {/* Header */}
        <div className="bg-blue-500 p-2 sm:p-3 lg:p-4 text-white grid grid-cols-2 md:grid-cols-4 gap-2 items-center justify-items-center shadow-md z-10 shrink-0">
          <div className="flex flex-col items-center">
            <span className="text-blue-200 text-[10px] sm:text-xs font-bold uppercase tracking-wider">Palabra</span>
            <span className="text-base sm:text-xl lg:text-2xl font-black">{gameStats.attempts}/10</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-blue-200 text-[10px] sm:text-xs font-bold uppercase tracking-wider">Aciertos</span>
            <span className="text-base sm:text-xl lg:text-2xl font-black flex items-center gap-1">✅ {gameStats.wordsCompleted}</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-blue-200 text-[10px] sm:text-xs font-bold uppercase tracking-wider">Puntos</span>
            <span className="text-base sm:text-xl lg:text-2xl font-black flex items-center gap-1">🌟 {gameStats.points}</span>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-blue-200 text-[10px] sm:text-xs font-bold uppercase tracking-wider">Racha</span>
            <span className="text-base sm:text-xl lg:text-2xl font-black flex items-center gap-1">🔥 {gameStats.streak}</span>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col items-center p-2 sm:p-4 overflow-y-auto min-h-0">
          ...
          {/* 1. SECCIÓN IMAGEN */}
          <div className="flex-1 w-full flex flex-col items-center justify-center min-h-0 shrink basis-auto py-1">
            <div className="relative group">
              <div className="absolute -inset-2 bg-gradient-to-r from-blue-400 to-purple-400 rounded-2xl blur opacity-25 group-hover:opacity-50 transition duration-200"></div>
          ...
          <div className="w-full flex flex-col items-center gap-2 shrink-0 mt-auto pt-1">
            <div className="flex flex-wrap justify-center items-center gap-2 sm:gap-4 w-full">
              ...
              >
                <span>🔊</span> <span className="hidden sm:inline">{isListening ? "..." : "Escuchar"}</span>
              </button>
